...
data t (Value : Set) (Neutral : Set) : Set where
  VLam : (Value → Result Value) → t Value Neutral
  VNeutral : Neutral → t Value Neutral
  VStar : t Value Neutral
  VPi : Value → (Value → Result Value) → t Value Neutral

data u (Value : Set) (Neutral : Set) : Set where
  NFree : Name → u Value Neutral
  NApp : Neutral → Value → u Value Neutral

data Value where
  val : t Value Neutral -> Value

data Neutral where
  neutral : u Value Neutral -> Neutral
...
vfree n = val (VNeutral (neutral (NFree n)))
...
iEval (e :$: e') d =
  ...
  cEval e' d >>= λ v' →
  vapp v v'
iEval Star d = return (val VStar)
iEval (Pi t t') d =
  cEval t d >>= λ v →
  return (val (VPi v (\x → cEval t' (x ∷ d))))

vapp (val (VLam f)) v = f v
vapp (val (VNeutral n)) v = return (val (VNeutral (neutral (NApp n v))))
vapp _ _ = throwError "invalid application"

cEval (Inf i) d = iEval i d
cEval (Lam e) d = return (val (VLam (\x → cEval e (x ∷ d))))

iType0 = iType 0

iType i g (Ann e t) =
  (cType i g t (val VStar)) >>
  ((cEval t []) >>= λ t' →
  (cType i g e t') >>
  (return t'))

iType i g Star = return (val VStar)

iType i g (Pi t t') =
  cType i g t (val VStar) >>
  (cEval t [] >>= λ p →
  cType (i + 1) ((Local i , p) ∷ g) (cSubst 0 (Free (Local i)) t') (val VStar) >>
  return (val VStar))
...
quoteN i (val (VNeutral n)) =
  neutralQuote i n >>= λ n' →
  return (Inf n')

quoteN i (val VStar) = return (Inf Star)

quoteN i (val (VPi v f)) =
  quoteN i v >>= λ v' →
  f (vfree (Quote i)) >>= λ i' →
  quoteN (i + 1) i' >>= λ f' →
  return (Inf (Pi v' f'))

neutralQuote i (neutral (NFree x)) = return (boundFree i x)

neutralQuote i (neutral (NApp n v)) =
  neutralQuote i n >>= λ n' →
  quoteN i v >>= λ v' →
  return (n' :$: v')
